...
The introduction of neural ranking models in the mid 2010s expanded this structure. Learned models were added as additional stages to rescore candidates retrieved by traditional methods. As language models became capable of generating fluent text conditioned on retrieved passages, pipelines grew further to include explicit stages for context assembly and generation. By the early 2020s, inference pipelines had evolved into multi stage workflows involving query processing, retrieval, filtering, re ranking, context construction, and generation, with each stage introducing its own design decisions and tradeoffs.
...
A common misconception is that inference pipelines are fixed and linear. In practice, modern pipelines often include conditional branching, iterative retrieval, and feedback loops, where the output of one stage influences the behavior of earlier or later stages. Viewing pipelines as static sequences obscures the dynamic nature of inference time decision making.
...
Inference pipelines introduce complexity that can be difficult to manage, analyze, and optimize. Each stage carries its own assumptions, failure modes, and computational costs, and interactions between stages can produce behaviors that are not predictable from any single component in isolation.

Latency presents a persistent tradeoff. Adding stages such as query rewriting, re ranking, or iterative retrieval can improve the quality of information available for generation, but each additional step increases response time. Balancing responsiveness and retrieval thoroughness is a central challenge in pipeline design.

Error propagation is another significant concern. Failures in early stages, such as poorly formulated queries or missed retrievals, cascade through the pipeline and affect all subsequent stages. Because later components depend on earlier outputs, a single upstream error can compromise the final response in ways that are difficult to diagnose.
